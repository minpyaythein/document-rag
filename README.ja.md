...
- **公開デプロイ向けの安全策** — 1MB のアップロード上限に加え、クライアント単位のレート制限
  （ウィンドウあたりのPDF数と、PDFごとの質問数）を備え、サイドバーの使用状況メーターにリアルタイム
  表示します。制限はサーバー側で保持され、URL に持たせたクライアントIDをキーにするため、ページを
  再読み込みしてもリセットされません。上限に達するとアップローダーはロックされ、ウィンドウが
  空くと自動で解除されます。
...
| `DISCORD_ERROR_WEBHOOK_URL` | いいえ | 🚨 アプリ内エラー通知用の Discord Webhook。未設定なら通知オフ。[監視とアラート](#監視とアラート)を参照。 |
...
補完し合う2つのレイヤーで構成されています：

- **レイヤー1 — 外形監視（UptimeRobot）。** HTTP(s) の**キーワード**モニターが
  `https://document-rag-minpyaythein.streamlit.app/healthz` を5分ごとに監視し、キーワード
  `ok` が無ければ Discord の連絡先に通知します。`/_stcore/health` ではなく `/healthz` を使う
  こと — 前者は匿名アクセスを Streamlit の認証/起動リダイレクトに 303 で飛ばすため、素のモニター
  からは `ok` が見えません。5分間隔の ping はスリープ防止（keep-alive）も兼ねます。注意：
  `/healthz` は Streamlit Cloud のエッジ層が応答するため、*プラットフォームが生きている*ことは
  示しても、RAG 本体が正常に動くことは保証しません — そこを補うのがレイヤー2です。
- **レイヤー2 — アプリ内エラー通知（Discord）。** `discord_alert.py` が、本物の上流障害
  — Gemini の埋め込みエラーと Z.AI のチャットエラー — を Discord チャンネルへ転送します
  （同一シグネチャにつき5分に1回までスロットリング）。`DISCORD_ERROR_WEBHOOK_URL` を設定すると
  有効（未設定ならオフ）。ユーザー入力起因のケース（スキャン/画像のみの PDF）や Streamlit の
  制御フロー例外（停止/再実行）は、意図的に通知**しません**。
