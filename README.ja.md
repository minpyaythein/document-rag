...
- **公開デプロイ向けの安全策** — 2MB のアップロード上限に加え、IP単位のレート制限（ウィンドウ
...
`.streamlit/config.toml` が公開時の安全策を設定します：`maxUploadSize = 2`（MBの上限）と
...
想定するホストは **Render**（無料枠）です。Streamlit は常駐サーバーが必要で、本アプリは
ステートレス（FAISS はメモリ上）なため、データベースは不要です。
